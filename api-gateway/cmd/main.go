...
func main() {
	...
	client, err := service.NewGRPCClient("localhost:50051", "localhost:50052", "localhost:50053")
	if err != nil {
	...
	r.GET("/stats/user-orders/:user_id", func(c *gin.Context) {
		userID := c.Param("user_id")
		stats, err := client.StatisticsClient.GetUserOrdersStatistics(c, &proto.UserOrderStatisticsRequest{UserId: userID})
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
	...
	r.GET("/stats/user", func(c *gin.Context) {
		stats, err := client.StatisticsClient.GetUserStatistics(c, &proto.UserStatisticsRequest{})
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
